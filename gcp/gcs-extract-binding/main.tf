resource "google_storage_bucket_iam_member" "extract_role_binding" {
  ...
  condition {
    ...
    title      = "${var.stitcher_ai_sa_email} restricts access to base_path for data extract role"
  }
}
...
resource "google_storage_bucket_iam_member" "extract_role_list_member" {
  bucket = var.customer_gcs_bucket
